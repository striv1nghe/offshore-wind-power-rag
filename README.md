...
- 知识库问答：读取 `knowledge_base/` 中的 Markdown 文档，可选择 TF-IDF 或 embedding 检索相关片段后交给大模型生成回答。
...
- `TF-IDF 字符检索`：默认方式，启动快，适合关键词明确的问题。
...
- 支持混合检索，把 TF-IDF 的关键词召回和 embedding 的语义召回合并排序。
- 增加更多高误差工况筛选规则。
- 保存问答历史和误差解释记录。
- 支持多模型结果对比。
